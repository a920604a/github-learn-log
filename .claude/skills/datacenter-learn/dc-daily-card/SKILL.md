...
**字數：`wc -m` 落在 3000–10000**（2026-07-30 上調，原為 3000–4500）。

`wc -m` 會把 frontmatter、markdown 標記、英文與程式碼區塊全部算進去，所以這個數字比「中文正文字數」大不少。

- **下限 3000 是硬的**：低於此通常代表深度不足或漏了必含段落
- **上限 10000 是硬的**：只有超過此才需要考慮「這張卡是不是塞了兩個主題」→ 拆成兩張分兩天寫，並在 `backlog.md` 就地插入新項目
- **舒適區約 6000–9000。** 舊上限 4500 太緊，dc-02 與 dc-03 都因此被迫壓縮或拆分。**參考基準卡改為 `datacenter/devices/transformer.md`（dc-02，2026-07-30 擴寫後實測 9404）** —— 那就是目標密度。使用者明確表示**寧願多花時間也要把概念學透**：不要再為省字數犧牲深度，也不要為填滿而灌水

空間有餘裕時，加厚的優先順序：

1. **關鍵數字與計算** — 多做一個帶數字的實例演算，或補一個邊界情況
2. **動手練習** — 更完整的 dataclass 骨架、更明確的驗收表格、加分題
3. **對資料模型的意涵** — 把「這個欄位會約束到誰」講到具體
4. 六格保持精簡，它是索引不是正文

不可省的段落（真的要砍時砍六格，不要砍這五段）：**關鍵數字與計算 / 常見誤解 / 對資料模型的意涵 / 動手練習 / 自我檢核**。這五段是知識能不能留下來的關鍵。
...
- **為了守字數而砍深度** → 上限已是 10000（2026-07-30 上調），舒適區 6000–9000。壓縮五段必含內容換字數是本末倒置
- **真的超過 10000** → 那才是塞了兩個主題，拆成兩張卡分兩天
