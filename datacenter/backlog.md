...
- [ ] `dc-05c` NFPA 110 定期測試制度、30% 門檻與 wet stacking
      <!-- 2026-08-05 從 dc-05b 再拆出：合寫初稿 12348 字元，超過 10000 上限 -->
      <!-- 已備妥的素材：月測 30 min ≥30% standby 銘牌 或 排氣溫度法（兩條互斥路徑）；
           月測達不到 30% → 年度補充 load bank 50%/30min + 75%/60min 共 1.5 h；
           Level 1 每 36 月測試（依 Class 時長，>4 h 可於 4 h 終止，≥30%）；
           驗收測試 7.13.4.3.1 step load 30%/30min → 50%/30min → 100%/60min；
           wet stacking：<30% 積碳，治療需 ≥75% 連續數小時；
           分歧：NFPA 寫 30%，廠商普遍給 30–50%；
           實例演算：ESP 2750 kW × 30% = 825 kW vs 誤用 COP 2100 → 630 kW（差 195 kW）；
           IT 400 kW × PUE 1.4 = 560 kW = 20.4% → 需 1502.5 kW load bank；
           年度時數 6.0 + 1.5 + 1.33 ≈ 8.83 h（回寫 dc-05 的 200 h 配額） -->
      <!-- 待辦：順便修正 dc-04 卡片裡「每年 4 小時 ≥30% 銘牌負載測試」的說法，
           那是把「月測 30%」與「Level 1 的 36 月 4 小時測試」混成一句 -->
