...
**維護特性**：NFPA 110 要求每月帶載測試 30 分鐘（且 ATS 本身每月要操作一次、起動用的 ATS 逐月輪替），達不到 30% 銘牌時另加年度 load bank 補充測試 1.5 小時；Level 1 另有每 36 個月的長時測試。ATS 是**「不定期演練就等於沒有」的設備**——它 99.9% 時間不動作，唯一驗證方式是主動觸發。制度全貌見 [dc-05c](nfpa110-testing-and-wet-stacking.md)。

<!-- +2026-08-07 修正：原文寫「每年 4 小時 ≥30% 銘牌負載測試」，那是把「月測 30%」與「Level 1 每 36 月的 4 小時測試」混成一句 -->
